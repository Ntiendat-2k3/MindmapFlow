...
### 💡 Tại sao tôi sử dụng những công nghệ này?

1.  **Next.js 14 & Vercel Postgres:** Sự kết hợp hoàn hảo để triển khai dự án nhanh chóng, hỗ trợ mạnh mẽ cho việc truy xuất dữ liệu từ phía Server giúp trang web tải cực nhanh.
2.  **React Flow & Zustand:** * **Tác dụng:** React Flow xử lý UI của sơ đồ rất mạnh nhưng dữ liệu (Nodes/Edges) thay đổi liên tục. 
    * **Tại sao dùng Zustand:** Thay vì dùng Redux quá nặng nề, Zustand cung cấp giải pháp nhẹ hơn, giúp đồng bộ hóa dữ liệu giữa React Flow và các thành phần khác trong app mà không gây ra hiện tượng giật lag (re-render thừa).
3.  **Sass (SCSS):** Với một ứng dụng có giao diện phức tạp như Mindmap, Sass giúp tôi tổ chức mã nguồn CSS sạch sẽ, dễ bảo trì và mở rộng hơn so với CSS thuần.

### 🧠 Phương án ghi nhớ cho phỏng vấn:
Khi nhà tuyển dụng hỏi về Tech Stack, hãy nhớ theo luồng **"Luồng Dữ Liệu"**:
* **Hiển thị:** React + React Flow + Sass.
* **Điều phối dữ liệu:** Zustand.
* **Xử lý & Bảo mật:** Next.js + NextAuth.
* **Lưu trữ:** Vercel Postgres.
